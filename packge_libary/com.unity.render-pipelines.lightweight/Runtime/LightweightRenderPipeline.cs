...
namespace UnityEngine.Rendering.LWRP
{
    ...
    public sealed partial class LightweightRenderPipeline : RenderPipeline
    {
        ...
        static class PerCameraBuffer
        {
            ...
            public static int _clipToWorldMatrix;
        }
        ...
        // Light data is stored in a constant buffer (uniform array)
        // This value has to match MAX_VISIBLE_LIGHTS in Input.hlsl
        public static int curCameraIndex
        {
            get;
            private set;
        }
        public static int allCameraCount
        {
            get;
            private set;
        }
        ...
        static CameraBufferInfo[] cameraBuffers;
        ...
        public LightweightRenderPipeline(LightweightRenderPipelineAsset asset)
        {
            ...
            PerCameraBuffer._clipToWorldMatrix = Shader.PropertyToID("_clipToWorldMatrix");
        ...
        private CameraBufferInfo ResetCameraBuffer(int index, int width, int height)
        {
            if (cameraBuffers == null)
            {
                cameraBuffers = new CameraBufferInfo[2];
            }
            ...
            var buffer = cameraBuffers[curCameraIndex % 2];
            if (buffer == null)
            {
                buffer = new CameraBufferInfo();
                buffer.texWidth = width;
                buffer.texHeight = height;
                buffer.texture = new RenderTexture(width, height, 32, GetRenderTextureFormat());
                buffer.colorIdentifier = new RenderTargetIdentifier(buffer.texture.colorBuffer);
                buffer.depthIdentifier = new RenderTargetIdentifier(buffer.texture.depthBuffer);
                cameraBuffers[curCameraIndex % 2] = buffer;
            }
            else if(buffer.texWidth != width || buffer.texHeight != height)
            {
                if (buffer.texture != null)
                    GameObject.DestroyImmediate(buffer.texture);
                buffer.texWidth = width;
                buffer.texHeight = height;
                buffer.texture = new RenderTexture(width, height, 32, GetRenderTextureFormat());
                buffer.colorIdentifier = new RenderTargetIdentifier(buffer.texture.colorBuffer);
        ...
        private void UpdateCameraBuffer(Camera camera)
        {
            ...
            var buffer = ResetCameraBuffer(curCameraIndex, pixelWidth, pixelHeight);
            camera.targetTexture = buffer.texture;
        }

        public static CameraBufferInfo GetLastCameraBuffer()
        {
            if(curCameraIndex == 0)
                return null;
            return cameraBuffers[curCameraIndex - 1 % 2];
        }
        ...
        protected override void Render(ScriptableRenderContext renderContext, Camera[] cameras)
        {
            ...
            {
                if(allCameraCount > 1 && curCameraIndex < allCameraCount - 1)
                {
        ...
        public static void RenderSingleCamera(ScriptableRenderContext context, Camera camera)
        {
            ...
            if (additionalCameraData != null && additionalCameraData.invertCulling)
        ...
        static void InitializeCameraData(LightweightRenderPipelineAsset settings, Camera camera, LWRPAdditionalCameraData additionalCameraData, out CameraData cameraData)
        {
            ...
            else
            {
                ...
                cameraData.requiresOpaqueTexture = settings.supportsCameraOpaqueTexture || cameraData.isSceneViewCamera;
            }
        ...
        static void SetupPerCameraShaderConstants(CameraData cameraData)
        {
            ...
            Matrix4x4 clipToWorldMatrix = camera.cameraToWorldMatrix * projMatrix.inverse;
            Shader.SetGlobalMatrix(PerCameraBuffer._clipToWorldMatrix, clipToWorldMatrix);
        }
