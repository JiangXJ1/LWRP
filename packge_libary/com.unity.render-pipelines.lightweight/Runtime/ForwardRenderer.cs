namespace UnityEngine.Rendering.LWRP
{
    internal class ForwardRenderer : ScriptableRenderer
    {
        ...
        RenderOpaqueBloomPass m_OpaqueBloomPass;
        DrawSkyboxPass m_DrawSkyboxPass;
        CopyDepthPass m_CopyDepthPass;
        CopyColorPass m_CopyColorPass;
        //CopyTmpColorPass m_CopyToTmpColorPass;
        CopyTmpColorPass m_CopyFromTmpColorPass;
        ...
        public ForwardRenderer(ForwardRendererData data) : base(data)
        {
            ...
            m_OpaqueBloomPass = new RenderOpaqueBloomPass(RenderPassEvent.AfterRenderingOpaques, RenderQueueRange.opaque, data.opaqueLayerMask);
            ...
            //m_CopyToTmpColorPass = new CopyTmpColorPass(RenderPassEvent.AfterRenderingPostProcessing);
            m_CopyFromTmpColorPass = new CopyTmpColorPass(RenderPassEvent.BeforeRenderingOpaques);
#if UNITY_EDITOR
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            if (LightweightRenderPipeline.curCameraIndex >= 1)
            {
                m_CopyFromTmpColorPass.Setup(LightweightRenderPipeline.GetLastCameraBuffer().colorIdentifier, m_ActiveCameraColorAttachment.Identifier());
                EnqueuePass(m_CopyFromTmpColorPass);
            }

            EnqueuePass(m_RenderOpaqueForwardPass);
            if (renderingData.cameraData.postProcessEnabled)
                EnqueuePass(m_OpaqueBloomPass);
            ...
            if (afterRenderExists)
            {
                ...
                {
                    //if (LightweightRenderPipeline.curCameraIndex < LightweightRenderPipeline.allCameraCount - 1)
                    //{
                    //    m_PostProcessPass.Setup(m_TmpColorDescriptor, m_ActiveCameraColorAttachment, m_TmpColorHandle);
                    //}
                    //else
                    {
                ...
                //else if (LightweightRenderPipeline.curCameraIndex < LightweightRenderPipeline.allCameraCount - 1)
                //{
                //    m_CopyToTmpColorPass.Setup(m_ActiveCameraColorAttachment.Identifier(), m_TmpColorIdentifier);
                //    EnqueuePass(m_CopyToTmpColorPass);
                //    Debug.LogError("Copy To TmpColor");
                //}
            ...
            else
            {
                if (postProcessEnabled)
                {
                    //if (LightweightRenderPipeline.curCameraIndex < LightweightRenderPipeline.allCameraCount - 1)
                    //{
                    //    m_PostProcessPass.Setup(cameraTargetDescriptor, m_ActiveCameraColorAttachment, m_TmpColorHandle);
                    //}
                    //else
                    {
